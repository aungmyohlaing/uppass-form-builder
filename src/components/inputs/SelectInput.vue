<template>
  <div class="form-field animate-fade-in" :class="layoutClass">
    ...
    <select
      ...
      @change="handleChange"
      @blur="handleBlur"
      class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 bg-white"
      :class="{
        'border-red-500': shouldShowError,
        'border-gray-300': !shouldShowError
      }"
    >
      <option value="">{{ field.display.placeholder || 'Select an option...' }}</option>
      <option
