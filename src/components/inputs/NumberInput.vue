<template>
  <div class="form-field animate-fade-in" :class="layoutClass">
    ...
    <input
      ...
      :max="field.value_constraints?.maximum"
      :min="field.value_constraints?.minimum"
      :step="field.value_constraints?.allow_decimal === 0 ? '1' : 'any'"
    ...
    <div v-if="field.value_constraints" class="mt-1 text-xs text-gray-400">
      <span v-if="field.value_constraints.minimum !== undefined">
        Min: {{ field.value_constraints.minimum }}
      </span>
      <span v-if="field.value_constraints.maximum !== undefined" class="ml-2">
        Max: {{ field.value_constraints.maximum }}
      </span>
      <span v-if="field.value_constraints.allow_decimal === 0" class="ml-2">
        (Whole numbers only)
      </span>
    </div>
  </div>
